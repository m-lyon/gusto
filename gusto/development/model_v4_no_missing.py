'''Gusto model prediction using single CpG site timepoints with no missing data in the training data'''
...
from gusto.lib.data import GustoSingleCpGDataset, GustoSingleCpGDatasetZeroMissing
...
class LITAutoEncoder(LITModel):
    ...
    def __init__(self):
        ...
        self.model = torch.nn.Sequential(
            LinearLayer(3, 8), LinearLayer(8, 8), torch.nn.Linear(8, 1)
        )
...
if __name__ == '__main__':
    ...
    train_dataset = GustoSingleCpGDatasetZeroMissing(
        Path.home().joinpath(
