...
def run_step_one(latent_dim, test_name):
    ...
    dataset = DictionaryDataset(TRAINING_DATA_NOMISSING, x_indices=[0, 1, 2, 3])
    train_alpha = Alpha(input_num=dataset.input_data.shape[0], latent_dim=latent_dim)
    model = DictLearning(train_alpha, train_alpha_only=False)
    trainer = L.Trainer(
        max_epochs=1000,
        accelerator='gpu',
        devices=1,
...
def run_step_two(latent_dim, test_name):
    ...
    dataset = DictionaryDataset(VALIDATION_DATA_NOMISSING, x_indices=[0, 1, 2])
    train_alpha = Alpha(input_num=52, latent_dim=latent_dim)
...
def run_step_three(latent_dim, test_name):
    ...
    train_dict = DictLearning.load_from_checkpoint(  # pylint: disable=no-value-for-parameter
        checkpoint_path=f'checkpoints/{test_name}_{latent_dim}/step_one.ckpt',
        alpha=Alpha(input_num=52, latent_dim=latent_dim),
    ...
    val_dict = DictLearning.load_from_checkpoint(  # pylint: disable=no-value-for-parameter
        ...
        alpha=Alpha(input_num=6, latent_dim=latent_dim),
        train_alpha_only=True,
    )
    train_dataset = DictionaryLSTMDataset(
        13, train_dict.alpha.alpha_params.detach(), TRAINING_DATA_NOMISSING
    )
    val_dataset = DictionaryLSTMDataset(
        2, val_dict.alpha.alpha_params.detach(), VALIDATION_DATA_NOMISSING
    )
    ...
    trainer = L.Trainer(
        ...
        devices=1,
        log_every_n_steps=1,
        logger=get_logger(f'{test_name}_step3', latent_dim),
        callbacks=[
