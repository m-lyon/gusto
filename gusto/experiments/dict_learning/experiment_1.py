...
def run_step_two(latent_dim, name, num_subjects, dataset_fpath: Path):
    ...
    model = DictLearning.load_from_checkpoint(  # pylint: disable=no-value-for-parameter
        checkpoint_path=f'checkpoints/{name}_{latent_dim}/step_one.ckpt',
        alpha=train_alpha,
...
def run_step_three(latent_dim, name, lstm_class, train_num_subjects, val_num_subjects):
    ...
    val_dict = DictLearning.load_from_checkpoint(  # pylint: disable=no-value-for-parameter
        checkpoint_path=f'checkpoints/{name}_{latent_dim}/step_two.ckpt',
        alpha=Alpha(input_num=val_num_subjects * 4, latent_dim=latent_dim),
        latent_dim=latent_dim,
        train_alpha_only=True,
    )
    train_dataset = DictionaryLSTMDataset(
        train_num_subjects, train_dict.alpha.alpha_params.detach(), TRAINING_DATA
    )
    val_dataset = DictionaryLSTMDataset(
        val_num_subjects, val_dict.alpha.alpha_params.detach(), VALIDATION_DATA
    )
...
if __name__ == '__main__':
    ...
    run_step_three(latent_size, test_name, SimpleLSTMLearning, num_train, num_val)
